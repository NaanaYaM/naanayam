...
int NaanayamUnits::decimals(int unit)
{
    switch(unit)
    {
    case BTC: return 6;
...
QString NaanayamUnits::format(int unit, qint64 n, bool fPlus)
{
...
QString NaanayamUnits::formatWithUnit(int unit, qint64 amount, bool plussign)
{
    return format(unit, amount, plussign) + QString(" ") + name(unit);
}

bool NaanayamUnits::parse(int unit, const QString &value, qint64 *val_out)
{
...
int NaanayamUnits::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return unitlist.size();
}

QVariant NaanayamUnits::data(const QModelIndex &index, int role) const
{
    int row = index.row();
