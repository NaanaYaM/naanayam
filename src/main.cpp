...
unsigned int nStakeMinAge = 60 * 60 * 24 * 90; //90 days minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 180; //180 days stake age of full weight
unsigned int nStakeTargetSpacing = 10; // 10 seconds block spacing
const int64 nChainStartTime = 1380478387; 
const int64 nTestNetStartTime = nChainStartTime; 
int nCoinbaseMaturity = 80; // minted blocks need need 100 confirm
...
const string strMessageMagic = "Naanayam Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // Naanayam: reward for coin-year is cut in half every 64x multiply of PoS difficulty
...
static const int64 nTargetTimespan = 0.125 * 24 * 60 * 60;  
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Naanayam miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Naanayam
    bool fStrictPayToScriptHash = true; // Always active in Naanayam
    ...
    // ppcoin: fees are not collected by miners as in Naanayam
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Naanayam", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "September 29th, 2013";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:NYM genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x61d0d51b85f249fb5d36540bef5b70b4975f4301a89180baf0fa19a750ce495d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe8, 0xe4, 0xe8, 0xe9 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("NaanayamMiner : proof-of-work not meeting target");

    //// debug print
    printf("NaanayamMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NaanayamMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("NaanayamMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadNaanayamMiner(void* parg);

static bool fGenerateNaanayams = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void NaanayamMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("Naanayam-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateNaanayams || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateNaanayams) && !fProofOfStake)
                return;
        }
        ...
        printf("Running NaanayamMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadNaanayamMiner(void* parg)
{
    ...
    try
    {
        ...
        NaanayamMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadNaanayamMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadNaanayamMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadNaanayamMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateNaanayams(bool fGenerate, CWallet* pwallet)
{
    fGenerateNaanayams = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateNaanayams = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d NaanayamMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadNaanayamMiner, pwallet))
                printf("Error: NewThread(ThreadNaanayamMiner) failed\n");
            Sleep(10);
        }
    }
}
//r3wt wuz here
//gb scripting by shakezula